...
for test in src/test/deca/syntax/invalid/*.deca
do
    resultat=$(test_synt "$test" 2>&1)
...
for test in src/test/deca/syntax/valid/*.deca
do
    fichier=$(echo ${test})
    resultat=$(test_synt "$test" 2>&1)
    if echo "$resultat" | grep -q "$fichier"':[0-9]'
    then
            echo -e "$test" ": ${ROUGE} Erreur syntaxique inattendue${BLANC}"
            return_status=1
